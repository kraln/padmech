...
void hw_init()
{
  ...
  /* Enable data pin as output */
  nrf_gpio_cfg_output(SNES_DATA);

  /* Enable pull-down on latch and clock pins */
  nrf_gpio_cfg_input(SNES_CLOCK, NRF_GPIO_PIN_PULLDOWN);
  nrf_gpio_cfg_input(SNES_LATCH, NRF_GPIO_PIN_PULLDOWN);

  /* Enable pull-ups on Button pins */
  nrf_gpio_cfg_input(BTN_DU,     NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_DL,     NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_DR,     NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_DD,     NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_SELECT, NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_START,  NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_B,      NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_A,      NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_Y,      NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_X,      NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_R,      NRF_GPIO_PIN_PULLUP); 
  nrf_gpio_cfg_input(BTN_L,      NRF_GPIO_PIN_PULLUP); 
...
void hw_latch_interrupt(bool enable)
{
   nrf_gpio_cfg_sense_input(
     ...
     (enable?NRF_GPIO_PIN_SENSE_HIGH:NRF_GPIO_PIN_NOSENSE)
   );
}
